...
export default async function SingleProductPage(props: {
	...
	const CommerceNoCache = Commerce.provider({
		tagPrefix: undefined,
		secretKey: undefined,
		cache: "no-store",
	});
	const searchResultNoCache = await CommerceNoCache.products.search({
		query: `active:'true' AND metadata["slug"]:'${params.slug}'`,
	});
	const latestVariant = searchResultNoCache.data.find(
		(variant: any) => variant.metadata.variant === selectedVariant,
	);
	const latestImage = latestVariant?.images?.[0] || product.images[0];
	const productWithLatestImage = {
		...product,
		images: [latestImage, ...product.images.slice(1)].filter((img): img is string => typeof img === "string"),
	};
	...
	return (
		<article className="pb-12">
			...
			<StickyBottom product={productWithLatestImage} locale={locale}>
				<div className="mt-4 grid gap-4 lg:grid-cols-12">
					<div className="lg:col-span-5 lg:col-start-8">
						<h1 className="text-3xl font-bold leading-none tracking-tight text-foreground">
							{productWithLatestImage.name}
						</h1>
						{productWithLatestImage.default_price.unit_amount && (
							<p className="mt-2 text-2xl font-medium leading-none tracking-tight text-foreground/70">
								{formatMoney({
									amount: productWithLatestImage.default_price.unit_amount,
									currency: productWithLatestImage.default_price.currency,
									locale,
								})}
							</p>
						)}
						<div className="mt-2">
							{productWithLatestImage.metadata.stock <= 0 && <div>Out of stock</div>}
						</div>
					</div>
					...
					<div className="lg:col-span-7 lg:row-span-3 lg:row-start-1">
						...
						<div className="grid gap-4 lg:grid-cols-3 [&>*:first-child]:col-span-3">
							...
							{productWithLatestImage.images.map((image, idx) => {
								...
								return (
									<YnsLink key={idx} href={`?${params}`} scroll={false}>
										{idx === 0 && !productWithLatestImage.metadata.preview ? (
											<MainProductImage
												key={image}
					...
					<div className="grid gap-8 lg:col-span-5">
						<section>
							...
							<div className="prose text-secondary-foreground">
								<Markdown source={productWithLatestImage.description || ""} />
							</div>
						</section>
						...
						<AddToCartButton
							productId={productWithLatestImage.id}
							disabled={productWithLatestImage.metadata.stock <= 0}
						/>
					</div>
				</div>
			</StickyBottom>
			...
			<Suspense>
				<SimilarProducts id={productWithLatestImage.id} />
			</Suspense>

			<Suspense>
				<ProductImageModal images={productWithLatestImage.images} />
			</Suspense>

			<JsonLd jsonLd={mappedProductToJsonLd(productWithLatestImage)} />
		</article>
	);
}
